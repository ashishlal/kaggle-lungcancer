...
from keras.layers import merge, Conv2D, MaxPooling2D, Input, UpSampling2D
...
validate_model_weights = 'unet_%d_validate.hdf5' % img_width
validate_model = 'model_validate_unet_%d.h5' % img_width

train_model_weights = 'unet_%d_train.hdf5' % img_width
train_model = 'model_train_unet_%d.h5' % img_width
...
def get_validation_data():
    my_train_imgs_fname = 'my_train_image_%d_%d.npy' % (img_width, img_height)
    my_train_masks_fname = 'my_train_mask_%d_%d.npy' % (img_width, img_height)
    my_test_imgs_fname = 'my_test_image_%d_%d.npy' % (img_width, img_height)
    my_test_masks_fname = 'my_test_mask_%d_%d.npy' % (img_width, img_height)
...
def get_unet_gen_model():
    inputs = Input((1, img_height, img_width))
    # output W2 = 256 x 256 x 32
    ...
    # output of UpSampling2D(size=(2, 2, 2))(conv7): 128 x 128 x 128
    # outputof up8: 128  x 128  x 192
    ...
    # output of UpSampling2D(size=(2, 2, 2))(conv8): 256 x 256 x 64
    # outputof up9: 256 x 256 x 96
...
def train_unet_model_for_validation(model, train_x, train_y, use_existing = False):
...
def predict_for_validation(model, test_x, test_y, use_existing = False):
    ...
    print('-'*30)
    num_test = len(test_x)
    imgs_mask_test = np.ndarray([num_test,1,img_height,img_width],dtype=np.float32)
...
def save_unet_validation(model):
...
def get_training_data():
    my_train_img_fname = 'my_train_image_%d_%d.npy' % (img_width, img_height)
    my_train_mask_fname = 'my_train_mask_%d_%d.npy' % (img_width, img_height)
...
def train_unet(model, train_x, train_y):
    ...
    print('-'*30)
    model.fit(train_x, train_y, batch_size=2, nb_epoch=20, verbose=2, shuffle=True,
              callbacks=[model_checkpoint])
    return model
...
def do_validation_and_training():
    ...
    model = get_unet_gen_model()
    model = train_unet_model_for_validation(model, train_x, train_y)
    predict_for_validation(model, test_x, test_y)
    save_unet_validation(model)
    train_x, train_y = get_training_data()
    model = train_unet(model, train_x, train_y)
    save_unet(model)
