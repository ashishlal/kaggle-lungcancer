...
json_file = open(model_path+'model_unet3d.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
model = model_from_json(loaded_model_json)

# load weights into new model
model.load_weights(model_path+"model_unet3d.h5")
...
num_zeros = 0
num_test = len(df)
segmented_lung_slices = np.ndarray([num_test,1,16,128,128],dtype=np.float32)
for i, row in tqdm(df.iterrows(), total=len(df)):
    ...
    try:
        ...
        X_segmented_lung_fname = working_path+'cache/X_segmented_lungs_%s_%s_%s_%s.npy' % (scan_folder, HM_SLICES, IMG_PX_SIZE, IMG_PX_SIZE)
