...
if __name__ == '__main__':
    ...
    df = pd.read_csv('data/stage1/stage1_sample_submission.csv')
...
#            continue
        ...
        try:
            ...
            X_segmented_images_fname = 'cache/X_test_segmented_lungs_%s_%s_%s_%s.npy' % (scan_folder, HM_SLICES, IMG_PX_SIZE, IMG_PX_SIZE)
...
#     for i, row in tqdm(df.iterrows(), total=len(df)):
#         scan_folder = row['scan_folder']
#         X_segmented_lungs_fname = 'cache/X_test_segmented_lungs_%s_%s_%s_%s.npy' % (scan_folder, HM_SLICES, IMG_PX_SIZE, IMG_PX_SIZE)
...
#             my_fname = 'cache/X_test_segmented_lung_%s_%d_%d_%d' % (scan_folder,j,img_width, img_height)
