...
model_arch = 'model_vnet_%d.json' % img_width
model_weights = 'model_vnet_%d.h5' % img_width
...
num_zeros = 0
num_test = len(df)
segmented_lung_slices = np.ndarray([1,1,num_slices,img_width,img_height],dtype=np.float32)
for i, row in tqdm(df.iterrows(), total=len(df)):
    ...
    try:
        ...
        my_nodules_mask = model.predict([segmented_lung_slices[0:1]], verbose=0)[0]
        #print '2..'
...
num_test = len(df)
#print num_test
test_lung_slices = np.ndarray([1,1,num_slices,img_width,img_height],dtype=np.float32)
for i, row in tqdm(df.iterrows(), total=len(df)):
#     if i != 0:
#        continue
    fname = os.path.join('data/stage1/stage1/', row['scan_folder'])
    ...
    try:
        ...
        if nz == 0:
            ...
            continue
        #print '1..'
        test_lung_slices[0,0] = segmented_lung
        my_nodules_mask = model.predict([test_lung_slices[0:1]], verbose=0)[0]
        #print '2..'
